...
public class C10nAdapter {
    ...
    public static List<ConversationResp> batchBuildC10nResp(Map<Long, C10nBaseInfo> c10nBaseInfoMap,
        ...
        return c10nBaseInfoMap.values().stream().map(c10nBaseInfo -> {
                    ...
                    if (Objects.nonNull(message)) {
                        ...
                        // 组装消息显示内容
                        resp.setLastMsgText(buildLastMsgSenderName(c10nBaseInfoMap, senderInfoMap, message)
                                + msgHandler.showMsgOnContact(message));
                ...
                // 按照会话活跃时间排序，最新的最前
                .sorted(Comparator.comparing(ConversationResp::getActiveTime).reversed())
     ...
     * @return {@link String }
     */
    private static String buildLastMsgSenderName(Map<Long, C10nBaseInfo> c10nBaseInfoMap,
                                                 Map<Long, UserEntity> senderInfoMap,
                                                 MessageEntity message) {
        UserEntity senderInfo = senderInfoMap.get(message.getSenderId());
        C10nBaseInfo c10nBaseInfo = c10nBaseInfoMap.get(message.getRoomId());
        if (senderInfo == null || c10nBaseInfo.getType().equals(RoomTypeEnum.FRIEND.getType())) {
            // 系统消息&单聊，不显示发送人
            return "";
        }
